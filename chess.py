...
sidebg = pygame.image.load("assets/sidemenu.jpg").convert()
player = 1
myfont = pygame.font.Font("assets/Roboto-Black.ttf", 30)
cpu = pygame.image.load("assets/cpu.png").convert_alpha()
cpu = pygame.transform.scale(cpu, (320, 240))
playeravatar = None
    ...
    while not gameover:
        ...
        if player == 1:
                ...
                if event.type == pygame.QUIT:
                    ...
                    if square in player_moves:
                        ...
                        if (board.white_king.y, board.white_king.x) not in attacked:
                            selected = False
                            player = "AI"
                            update_sidemenu("ИИ думает...", (255, 255, 255))
                            if dest:
                            ...
                            piece.highlight()
                            if checkWhite:
                                update_sidemenu("Шах", (255, 0, 0))
                                pygame.display.update()
                                pygame.time.wait(1000)
                                update_sidemenu("Ваш ход!", (255, 0, 0))
                            else:
                                update_sidemenu("Шах", (255, 0, 0))
                                pygame.display.update()
                                pygame.time.wait(1000)
                        ...
                        if (special == "CR" or special == "CL") and type(piece) == King:
                            ...
                            update_sidemenu("Невозможный ход", (255, 0, 0))
                            pygame.display.update()
                            pygame.time.wait(1000)
                            if checkWhite:
                                update_sidemenu("Шах!", (255, 0, 0))
            ...
            if value == float("-inf") and move == 0:
                ...
                player = 1
                update_sidemenu(
                    "Вы выиграли. \n Нажмите любую кнопку что бы выйти.", (255, 255, 0)
                )
            else:
                start = move[0]
                end = move[1]
...
def game_over():
    ...
    winner = pygame.image.load("assets/winner.png").convert_alpha()
    winner = pygame.transform.scale(winner, (80, 60))
    screen.blit(winner, (520, 20))
    pygame.display.update()
...
def update_sidemenu(message, colour):
    ...
    global playeravatar, cpu
    if player == 1:
        screen.blit(playeravatar, (480, 0))

    elif player == "AI":
        screen.blit(cpu, (480, 0))
...
def welcome():
    ...
    textsurface = medfont.render("StailFX project", False, (255, 255, 255))
    screen.blit(textsurface, (240, 100))
    textsurface = myfont.render("Press any key to begin!", False, (255, 255, 255))
